...
def decayheat():
    ...
    for i, y in ind.items():
        ...
        for t in time:
            ...
            for k in dd_dict.keys():
                ...
                fraction = y * branch_prod  # Indepenent yield is normalized to 200%, fraction == N0 in bateman
                ...
                '''
                X = bateman(dd_dict[k]['lmbds'], fraction, t)
                ...
                b = [xi * float(enb)/1E+6 * lm for xi, enb, lm in zip(X, dd_dict[k]['en_betas'], dd_dict[k]['lmbds'])]
                g = [xi * float(eng)/1E+6 * lm for xi, eng, lm in zip(X, dd_dict[k]['en_gamms'], dd_dict[k]['lmbds'])]
                a = [xi * float(ena)/1E+6 * lm for xi, ena, lm in zip(X, dd_dict[k]['en_alphas'], dd_dict[k]['lmbds'])]
    ...
    oyak = pd.read_csv("sample/decayheat-2.53E-8-mev.dat", sep='\s+', header=0, comment='#')
